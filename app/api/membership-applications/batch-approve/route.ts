 ...
 * Batch approve multiple membership applications
 */
export async function POST(req: NextRequest) {
  ...
  // 2. Role authorization - only superadmin and editor can approve
  ...
  try {
    ...
    const result = await db.membershipApplication.updateMany({
      where: {
        id: { in: ids },
        status: "pending", // Only approve pending applications
      },
      data: {
        status: "approved",
        reviewedAt: new Date(),
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully approved ${result.count} applications`,
