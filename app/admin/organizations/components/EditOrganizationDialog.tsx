...
export function EditOrganizationDialog({
  ...
  const [form, setForm] = React.useState<EditFormState>(initialFormState);
  ...
  const showRegion = React.useMemo(
    () => ["dpd", "dpc", "dprt", "kader"].includes(form.level),
    [form.level]
  );

  const showSayap = React.useMemo(() => form.level === "sayap", [form.level]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.useFileUpload && form.photoFile) {
      setUploading(true);
      const uploadedUrl = await handleFileUpload(form.photoFile);
      setUploading(false);
              ...
              <section className="space-y-5">
                ...
                <div className="space-y-4">
                  <div className="flex h-48 w-full items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-gray-300 bg-gradient-to-br from-gray-50 to-gray-100">
                    {form.useFileUpload && form.photoFile ? (
                      <img
                  ...
                  <Tabs
                    value={form.useFileUpload ? "upload" : "url"}
                    onValueChange={(value) =>
                      setForm((current) => ({
                        ...current,
              ...
              {(showRegion || showSayap) && (
                <section className="space-y-5">
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            setForm((current) => ({
                              ...current,
                              regionId: value === "none" ? null : Number(value),
                            }))
                          }
                        >
                          <SelectTrigger className="h-11 rounded-xl border-[#001B55]/15 bg-white/90 shadow-sm w-full">
                    ...
                    {showSayap && (
                      <div className="space-y-2">
                        <Label className="text-xs text-gray-600">Sayap</Label>
                        <Select
                          value={
